 ...
 * @license AGPL-3.0
 */
class CRM_Streetimport_Config_OptionValue {

  protected $_apiParams = array();

  /**
   * CRM_Streetimport_Config_OptionValue constructor.
   */
  ...
  protected function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name'])) {
      throw new Exception('Missing mandatory param name in class CRM_Streetimport_Config_OptionValue');
    }
    if (!isset($params['option_group_id']) || empty($params['option_group_id'])) {
      throw new Exception('Missing mandatory param option_group_id in class CRM_Streetimport_Config_OptionValue');
    }
