 ...
 * @license AGPL-3.0
 */
class CRM_Streetimport_Config_RelationshipType {

  protected $_apiParams = array();

  /**
   * CRM_Streetimport_Config_RelationshipType constructor.
   */
  ...
  private function validateCreateParams($params) {
    if (!isset($params['name_a_b']) || empty($params['name_a_b']) ||
      ...
      throw new Exception('When trying to create a Relationship Type name_a_b and name_b_a are mandatory
        parameter and can not be empty in class CRM_Streetimport_Config_RelationshipType');
    }
  ...
  public function create($params) {
    ...
    if (!isset($this->_apiParams['label_a_b']) || empty($this->_apiParams['label_a_b'])) {
      $this->_apiParams['label_a_b'] = CRM_Streetimport_Utils::buildLabelFromName($this->_apiParams['name_a_b']);
    }
    if (!isset($this->_apiParams['label_b_a']) || empty($this->_apiParams['label_b_a'])) {
      $this->_apiParams['label_b_a'] = CRM_Streetimport_Utils::buildLabelFromName($this->_apiParams['name_b_a']);
    }
    try {
