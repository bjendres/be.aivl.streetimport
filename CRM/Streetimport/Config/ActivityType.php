 ...
 * @license AGPL-3.0
 */
class CRM_Streetimport_Config_ActivityType extends CRM_Streetimport_Config_OptionValue {
  /**
  ...
  protected function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name'])) {
      throw new Exception('Missing mandatory param name in class CRM_Streetimport_Config_ActivityType');
    }
    $this->_apiParams = $params;
    try {
      $this->_apiParams['option_group_id'] = $this->getOptionGroupId();
    } catch (CiviCRM_API3_Exception $ex) {
      throw new Exception('Unable to find option group for activity_type in CRM_Streetimport_Config_ActivityType, contact your system administrator');
    }
  }
