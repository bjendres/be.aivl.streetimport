 ...
 * @license AGPL-3.0
 */
class CRM_Streetimport_Config_Group {

  protected $_apiParams = array();

  /**
   * CRM_Streetimport_Config_Group constructor.
   */
  ...
  private function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name'])) {
      throw new Exception('Missing mandatory param name in class CRM_Streetimport_Config_Group');
    }
  ...
  private function sanitizeParams() {
    ...
    if (empty($this->_apiParams['title']) || !isset($this->_apiParams['title'])) {
      $this->_apiParams['title'] = CRM_Streetimport_Utils::buildLabelFromName($this->_apiParams['name']);
    }
